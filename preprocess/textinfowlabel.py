...
class TextInfoWlabelDataset(BaseDataset):
    def __init__(self, cfg):
        super(TextInfoWlabelDataset, self).__init__(cfg)
        self.change_item_name()
        self.input_size = 400 + 23#text embedding 200
    ...
    def change_item_name(self):
        ...
        for t in tqdm(self.datalist):
            ...
            temp2 = []
            for item in t["item"]:
                temp1.append(prob_text_dict[item])
                temp2.append(prob_dict[item]["label"])
    ...
    def __getitem__(self, idx):
        ...
        inter_emb = torch.cat((inter_emb, torch.tensor(temp_dict["label"][:-1])),dim=-1)
        return dict(
